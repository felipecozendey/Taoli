...
export default function ClientProductivity() {
  ...
  const loadData = async (silent = false) => {
    ...
    try {
      const [habitsRes, tasksRes, logsRes] = await Promise.all([
        ...
        productivityService.getTasks(), // Busca todas as tarefas para o Kanban
  ...
  const completedItems = habitLogs.length + todaysTasks.filter((t) => t.status === 'done').length
      ...
      <PageContent className="max-w-6xl mx-auto w-full">
        ...
        <Tabs defaultValue="daily" className="w-full">
          <TabsList className="grid w-full grid-cols-2 max-w-[400px] mb-6">
            ...
            <TabsTrigger value="kanban">Planeamento (Kanban)</TabsTrigger>
          </TabsList>
